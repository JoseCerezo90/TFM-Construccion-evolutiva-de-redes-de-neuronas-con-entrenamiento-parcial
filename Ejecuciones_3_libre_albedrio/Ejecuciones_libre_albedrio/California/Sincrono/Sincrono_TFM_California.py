...
class Individual:
    ...
    def __init__(self, axiom, grammar, non_terminals, max_depth, current_depth, use_GUPI, starting, nn_data, flag_crossover):
        ...
        if flag_crossover:
            ...
            self.fitness = self.neural_model.run_and_test()
...
class Evolution:
    ...
    def __debug(self, it, debug_metric):
        ...
        return fitness_average, best_TOP_fitness
    ...
    def evolve(self):
        ...
        fitness_average, best_TOP_fitness = self.__debug(it, debug_metric)
        while not self.__GGGP_stop_condition(it, best_TOP_fitness, last_fitness_average):
            ...
            # Mutation
            self.__mutation()
            # Replacement (Elitist or SSGA)
            if self.use_elitist_repl: self.__replacement_elitist()
            else: self.__replacement_SSGA()
    ...
    def __roulette_wheel_selection(self):
        ...
        if self.OUTPUTS == 1:
            for i in range(self.population_size):
                sum_fitness += (1 / self.population[i].fitness)
            for i in range(self.population_size):
                fitness_list.append((1/self.population[i].fitness) / sum_fitness)
        # Classification Problem: Reward the MORE fitness (accuracy) you have
        else:
            for i in range(self.population_size):
                sum_fitness += self.population[i].fitness
            for i in range(self.population_size):
                fitness_list.append(self.population[i].fitness / sum_fitness)
        # Roulette partitions
        probs = []
        for i in range(self.individuals_to_select):
            probs.append((i+1)/(self.individuals_to_select+1))
    ...
    def __replacement_SSGA(self):
        population_eval = []
        # Evaluate population, not sons
        for i in range(self.population_size):
            population_eval.append((self.population[i].fitness, self.population[i]))
        ...
        best_individuals = sorted_population[:(self.population_size-len(self.sons))]
        # The new population is the one that remains + the children
        new_population = []
    ...
    # Elitist
    def __replacement_elitist(self):
        population_eval = []
        # Evaluate all the individuals
        for i in range(self.population_size):
            population_eval.append((self.population[i].fitness, self.population[i]))
        for i in range(len(self.sons)):
            population_eval.append((self.sons[i].fitness, self.sons[i]))
        # Sort the population based on fitness (Regression Problem reward the LOWER fitness, but classification Problem reward the MORE fitness)
        if self.OUTPUTS == 1: sorted_population = sorted(population_eval, key=lambda x: x[0], reverse=False)
        else: sorted_population = sorted(population_eval, key=lambda x: x[0], reverse=True)
        # Keep the best individuals
        best_individuals = sorted_population[:self.population_size]
        # The new population is the one that remains
        new_population = []
        for i in range(len(best_individuals)):
            new_population.append(best_individuals[i][1])
        self.population = new_population
...
if __name__ == "__main__":
    ...
    evo = Evolution(axiom=axiom, grammar=grammar, non_terminals=non_terminals, max_depth=max_depth, current_depth=current_depth, population_size=population_size, 
                    ...
                    max_iterations=max_iterations, nn_data=nn_data, use_elitist_repl=False)
    evo.evolve()

    time_2 = time.time()
    # Saving files
    n_exe = 24
    n_file_1 = './MedianHouseValue_Sync_Best_Fit_' + str(n_exe) + '.csv'
    n_file_2 = './MedianHouseValue_Sync_Aver_Fit_' + str(n_exe) + '.csv'
    n_file_3 = './MedianHouseValue_Sync_Epochs_' + str(n_exe) + '.csv'
    n_file_4 = './MedianHouseValue_Sync_TOP_Fit_' + str(n_exe) + '.csv'
    txt_file = 'MedianHouseValue_Sync_' + str(n_exe) + '.txt'
