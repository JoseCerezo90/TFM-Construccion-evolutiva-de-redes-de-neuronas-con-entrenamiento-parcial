...
class Evolution:
    ...
    # Population size = 40; Top individuals = 10 (Minimize)
    def __get_TOP_individuals(self):
        fitness_array = []
        for i in range(len(self.population)):
            fitness_array.append(self.population[i].fitness)
        # Minimize
        new_array = np.sort(fitness_array)
        # Top 10
        top_array = new_array[:10]
    ...
    def __GGGP_stop_condition(self, iteration, fitness_average, last_fitness_average):
        ...
        percent_improv = ((last_fitness_average/fitness_average)-1)*100
        if percent_improv < 0.0175: self.GGGP_not_improving += 1
...
class FFNN:
    ...
    def run_and_test(self, last_fitness):
        ...
        while (epochs_now < self.asyn_epochs) and keep_training:
            ...
            percent_improv = ((last_fitness/new_fitness)-1)*100 
...
if __name__ == "__main__":
    ...
    # Saving files
    n_exe = 24
    n_file_1 = './MedianHouseValue_Async_Best_Fit_' + str(n_exe) + '.csv'
    n_file_2 = './MedianHouseValue_Async_Aver_Fit_' + str(n_exe) + '.csv'
    n_file_3 = './MedianHouseValue_Async_Epochs_' + str(n_exe) + '.csv'
    n_file_4 = './MedianHouseValue_Async_TOP_Fit_' + str(n_exe) + '.csv'
    txt_file = 'MedianHouseValue_Async_' + str(n_exe) + '.txt'
