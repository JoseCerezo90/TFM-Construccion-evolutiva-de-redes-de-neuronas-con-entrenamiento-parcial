...
class Individual:
    # Constructor
    def __init__(self, axiom, grammar, non_terminals, max_depth, current_depth, use_GUPI, starting, nn_data, flag_crossover):
        self.axiom = axiom
        ...
        # If an individual is created by crossing or mutation, it is not necessary to train it before its corresponding representation is established
        if flag_crossover:
            self.decoded = self.__decode_individual()
            self.neural_model = FFNN(encoded_network=self.decoded, nn_data=self.nn_data)
            if self.OUTPUTS == 1: 
                self.fitness, _, _ = self.neural_model.run_and_test(1)    
            else: 
                self.fitness, _, _ = self.neural_model.run_and_test(0.01)   
        self.mature = False
...
class Evolution:
    ...
    crossover_prob, mutation_prob, use_GUPI, max_iterations, nn_data, use_elitist_repl):
        ...
        self.GGGP_not_improving = 0
        self.sons = []
        _, OUTPUTS, _, _, _, _, _, _ = self.nn_data
        self.OUTPUTS = OUTPUTS
        # Variables to store data and generate graphs
        self.array_best_fitness = []
        self.array_TOP_best_fitness = []
        self.array_average_fitness = []
        self.array_epochs_each_iteration = []

    # Population size = 20; Top individuals = 5 (Maximize)
    def __get_TOP_individuals(self):
        fitness_array = []
        for i in range(len(self.population)):
            fitness_array.append(self.population[i].fitness)
        # Maximize
        new_array = np.sort(fitness_array)[::-1]
        # Top 5
        top_array = new_array[:5]
        return np.mean(top_array)

    def debug(self, it, debug_metric):
        global total_epochs
        
        best_individual = self.get_best_individual()
        fitness_average = self.__get_fitness_average()

        best_TOP_fitness = self.__get_TOP_individuals()
        self.array_TOP_best_fitness.append(best_TOP_fitness)

        self.array_best_fitness.append(best_individual.fitness)
        self.array_average_fitness.append(fitness_average)
        self.array_epochs_each_iteration.append(total_epochs) 
        txt_chain = "Iteration " + str(it) + ". Best individual: " + str(best_individual.decoded) + ". Best fitness: " + \
                str(best_individual.fitness) + ". Population average: " + str(fitness_average) + debug_metric + ". TOP ind. fit.: " + str(best_TOP_fitness) + ". Epoch accum: " + str(total_epochs)
        print(txt_chain)
        return fitness_average, txt_chain, best_TOP_fitness
    ...
    def evolve(self):
        ...
        fitness_average, _, best_TOP_fitness = self.debug(it, debug_metric)
        while not self.__GGGP_stop_condition(it, best_TOP_fitness, last_fitness_average):
            last_fitness_average = best_TOP_fitness
            it += 1
            self.sons = []
            # Selection
            ...
            for i in range(self.population_size):
                if (self.population[i].to_be_trained) and (self.population[i].neural_model.current_epochs < self.population[i].neural_model.n_epochs):
                    last_fitness = self.population[i].fitness
                    self.population[i].fitness, mature, keep_training = self.population[i].neural_model.run_and_test(last_fitness)
                    if mature: self.population[i].mature = True
                    if not keep_training: 
                        self.population[i].to_be_trained = False
                        self.population[i].mature = True
    ...
    def __generate_random_population(self):
        while len(self.population) < self.population_size:
            i = Individual(axiom=self.axiom, grammar=self.grammar, non_terminals=self.non_terminals, max_depth=self.max_depth, 
                          current_depth=self.current_depth, use_GUPI=self.use_GUPI, starting=True, nn_data=self.nn_data, flag_crossover=True)
    ...
    def __create_son(self, son_representation):
        son = Individual(axiom=self.axiom, grammar=self.grammar, non_terminals=self.non_terminals, max_depth=1, 
                          current_depth=self.current_depth, use_GUPI=self.use_GUPI, starting=True, nn_data=self.nn_data, flag_crossover=False)
        ...
        for i in range(len(self.sons)):
            ...
            if random.uniform(0, 1) <= self.mutation_prob:
                ...
                mutated_i = Individual(axiom=axiom, grammar=self.grammar, non_terminals=self.non_terminals, max_depth=new_max_depth, 
                                      current_depth=current_depth, use_GUPI=self.use_GUPI, starting=False, nn_data=self.nn_data, flag_crossover=False)
    ...
    def __GGGP_stop_condition(self, iteration, fitness_average, last_fitness_average):
        if iteration == 0: return False
        percent_improv = ((fitness_average/last_fitness_average)-1)*100
        if percent_improv < 0.05: self.GGGP_not_improving += 1
        else: self.GGGP_not_improving = 0
        if self.GGGP_not_improving == 5: return True
        else: return False
...
class FFNN:
    ...
    def run_and_test(self, last_fitness):
        ...
        epochs_now = 0
        keep_training = True
        while (epochs_now < self.asyn_epochs) and keep_training:
            ...
            percent_improv = ((new_fitness/last_fitness)-1)*100 
            if percent_improv < 0.0175: self.BP_not_improving += 1 
            else: self.BP_not_improving = 0
            if self.BP_not_improving == 5: keep_training = False
        ...
        return new_fitness, percent_improv < 5, keep_training
...
if __name__ == "__main__":
    ...
    max_depth = 5                       # GUPI paper recursion (rho)
    ...
    mutation_prob = 0.05                # Probability of applying the mutation operator
    ...
    # Saving files
    n_exe = 20
    n_file_1 = './FIFA_Async_Best_Fit_' + str(n_exe) + '.csv'
    n_file_2 = './FIFA_Async_Aver_Fit_' + str(n_exe) + '.csv'
    n_file_3 = './FIFA_Async_Epochs_' + str(n_exe) + '.csv'
    n_file_4 = './FIFA_Async_TOP_Fit_' + str(n_exe) + '.csv'
    txt_file = 'FIFA_Async_' + str(n_exe) + '.txt'
    
    savetxt(n_file_1, evo.array_best_fitness, delimiter=',')
    savetxt(n_file_2, evo.array_average_fitness, delimiter=',')
    savetxt(n_file_3, evo.array_epochs_each_iteration, delimiter=',')
    savetxt(n_file_4, evo.array_TOP_best_fitness, delimiter=',')

    if nn_data[1] == 1: debug_metric = " (MSE)"
    else: debug_metric = " (Accuracy)"
    fitness_average, txt_chain1, _ = evo.debug(1000, debug_metric)

    # Finish training
    for i in range(population_size):
        while evo.population[i].to_be_trained and (evo.population[i].neural_model.current_epochs < evo.population[i].neural_model.n_epochs):          
            last_fitness = evo.population[i].fitness
            evo.population[i].fitness, _, keep_training = evo.population[i].neural_model.run_and_test(last_fitness)
            if not keep_training: evo.population[i].to_be_trained = False

    fitness_average, txt_chain2, _ = evo.debug(1000, debug_metric)
    ...
    cad4 = "--------------------------------------------------"
    lines = [cad1, cad2, cad3, cad4, txt_chain1, txt_chain2]
