...
class Evolution:
    ...
    def evolve(self):
        ...
        while not self.__stop_condition(it):
    ...
    def __stop_condition(self, iteration):
...
class FFNN:
    ...
    def run_and_test(self):
        ...
        # Regression problem
        if self.OUTPUTS == 1: return history.history['val_loss'][len(history.history['val_loss'])-1]
        # Classification problem
        else: return history.history['val_categorical_accuracy'][len(history.history['val_categorical_accuracy'])-1]
...
if __name__ == "__main__":
    ...
    # Executions that the algorithm is going to execute in order to average
    n_executions = 1
    # Evolution parameters (the parameters of the neural networks are in the FFNN class)
    max_iterations = 25             	                # Maximum iterations allowed for the evolutionary process
    current_depth = 0                                   # Initial depth from which the trees (individuals) are generated
    l_population_size = [20, 30, 40]                    # Number of individuals
    l_max_depth = [3, 4, 5]                             # GUPI paper recursion (rho)
    l_selection_percentage = [0.4, 0.5, 0.6]            # Percentage of individuals, with respect to the population size, that will be selected to subsequently outcrossing
    l_crossover_prob = [0.9, 0.95]                      # Probability of applying the crossover operator
    l_mutation_prob = [0.05, 0.1]                       # Probability of applying the mutation operator

    # List containing all combinations of hyperparameters
    combinations = list(itertools.product(l_population_size, l_max_depth, l_selection_percentage, l_crossover_prob, l_mutation_prob))
    ...
    for i in range(n_executions):
        for j in range(len(combinations)):
            if ((i*len(combinations)+j+1) > 83 and (i*len(combinations)+j+1) < 87):
                population_size, max_depth, selection_percentage, crossover_prob, mutation_prob = combinations[j]
                ...
                chain = str(combinations[j]) + ". Best fitness: " + str(evo.array_best_fitness[len(evo.array_best_fitness)-1]) + ". Average fitness: " + str(evo.array_average_fitness[len(evo.array_average_fitness)-1])
                lines.append(chain)
                print("Iteración " + str(i*len(combinations)+j+1) + " - " + chain + " | " + str( np.round(((i*len(combinations)+j+1) / (len(combinations)*n_executions))*100, 2)) + "%")

    if "FIFA" in file_1: txt_file = 'Hiperparametros_FIFA_Sync.txt'
    else: txt_file = 'Hiperparametros_MedianHouseValue_Sync.txt'
