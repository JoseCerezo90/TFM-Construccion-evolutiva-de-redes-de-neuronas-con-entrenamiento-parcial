...
class Evolution:
    ...
    def debug(self, it, debug_metric):
        ...
        txt_chain = "Iteration " + str(it) + ". Best individual: " + str(best_individual.decoded) + ". Best fitness: " + \
                str(best_individual.fitness) + ". Population average: " + str(fitness_average) + debug_metric + ". Epoch accum: " + str(total_epochs)
        print(txt_chain)
        return fitness_average, txt_chain
    ...
    def evolve(self):
        ...
        fitness_average, _ = self.debug(it, debug_metric)
...
class FFNN:
    ...
    def run_and_test(self, last_fitness):
        global total_epochs
        epochs_now = 0
        # Flag to signal if the second stop condition of reaching a very good adaptation value is reached
        keep_training = True
        
        while (epochs_now < self.asyn_epochs) and (self.current_epochs < self.n_epochs) and keep_training:
            ...
            if self.OUTPUTS == 1: 
                ...
                if new_fitness < 0.23: keep_training = False
            # Classification problem
            else: 
                ...
                if new_fitness > 0.825: keep_training = False
        ...
        return new_fitness, improvement_percentage < 5, keep_training
...
if __name__ == "__main__":
    ...
    evo = Evolution(axiom=axiom, grammar=grammar, non_terminals=non_terminals, max_depth=max_depth, current_depth=current_depth, population_size=population_size, 
                    selection_percentage=selection_percentage, crossover_prob=crossover_prob, mutation_prob=mutation_prob, use_GUPI=False, 
    ...
    fitness_average, txt_chain1 = evo.debug(1000, debug_metric)
    ...
    fitness_average, txt_chain2 = evo.debug(1000, debug_metric)
